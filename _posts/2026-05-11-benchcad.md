...
CAD（computer-aided design，计算机辅助设计）是几乎所有实体产品的起点：一台发动机、一个手机中框、一颗螺栓，量产之前都先在 CAD 里建成精确的参数化模型。把 CAD、CAE（仿真）、EDA（芯片设计）、PLM 这套工程设计与仿真软件放在一起看，是一门被少数寡头垄断、每一家又都极值钱的生意：EDA 两巨头 Synopsys、Cadence 市值各约 750 亿和 730 亿美元，Autodesk 约 410 亿，Dassault（SOLIDWORKS / CATIA）约 260 亿，PTC 约 165 亿——光这几家加起来就两千多亿美元，还没算 Siemens 这种把 NX 和整套仿真都揽进去的工业巨头。过去十年里软件工程几乎每一层都被 AI 渗透了一遍，但硬件设计这一端基本没动：工程师还是一根线、一个尺寸地手工建模，AI 顶多在仿真和排版上打打下手。
...
CAD (computer-aided design) is where almost every physical product begins: an engine, a phone mid-frame, a single bolt — all of it becomes an exact parametric model in CAD before anything gets manufactured. Take CAD, CAE (simulation), EDA (chip design), and PLM together and it’s a business owned by a few incumbents, each enormously valuable: the two EDA giants Synopsys and Cadence are worth about $75B and $73B, Autodesk about $41B, Dassault (SOLIDWORKS / CATIA) about $26B, PTC about $16.5B — over $230B among just these, before counting Siemens, which folds NX and a full simulation stack into a far larger industrial conglomerate. Over the past decade AI has worked its way into nearly every layer of software engineering, but the hardware-design end has barely moved: engineers still model line by line, dimension by dimension, with AI relegated to simulation and layout.
