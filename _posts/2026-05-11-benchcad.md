...
CAD（computer-aided design，计算机辅助设计）是几乎所有实体产品的起点：一台发动机、一个手机中框、一颗螺栓，量产之前都先在 CAD 里建成精确的参数化模型。这是个巨大且高度集中的产业：光 CAD 软件本身的年收入就在百亿美元量级，再把 CAE（仿真）、EDA（芯片设计）、PLM 一起算进这套工程设计与仿真软件栈，年收入达几百亿美元、主导厂商合计市值已是数千亿乃至逼近万亿美元——Dassault（SOLIDWORKS / CATIA）、Autodesk（AutoCAD / Fusion 360）、Siemens（NX）、PTC（Creo），再加 EDA 那头的 Synopsys、Cadence，几家寡头卡着绝大部分。过去十年里软件工程几乎每一层都被 AI 渗透了一遍，但硬件设计这一端基本没动：工程师还是一根线、一个尺寸地手工建模，AI 顶多在仿真和排版上打打下手。
...
CAD (computer-aided design) is where almost every physical product begins: an engine, a phone mid-frame, a single bolt — all of it becomes an exact parametric model in CAD before anything gets manufactured. It’s a huge, highly concentrated industry: CAD software alone is tens of billions in annual revenue, and once you fold in CAE (simulation), EDA (chip design), and PLM, the design-and-simulation software stack runs to many tens of billions a year, with the leading vendors carrying hundreds of billions — approaching a trillion — in combined market value. A handful of incumbents own most of it: Dassault (SOLIDWORKS / CATIA), Autodesk (AutoCAD / Fusion 360), Siemens (NX), PTC (Creo), plus Synopsys and Cadence on the EDA side. Over the past decade AI has worked its way into nearly every layer of software engineering, but the hardware-design end has barely moved: engineers still model line by line, dimension by dimension, with AI relegated to simulation and layout.
