...
CAD（computer-aided design，计算机辅助设计）是几乎所有实体产品的起点：一台发动机、一个手机中框、一颗螺栓，量产之前都先在 CAD 里建成精确的参数化模型。这是个百亿美元量级的市场，常年被少数几家寡头瓜分——Dassault（SOLIDWORKS / CATIA）、Autodesk（AutoCAD / Fusion 360）、Siemens（NX）、PTC（Creo）。过去十年里软件工程几乎每一层都被 AI 渗透了一遍，但硬件设计这一端基本没动：工程师还是一根线、一个尺寸地手工建模，AI 顶多在仿真和排版上打打下手。
...
CAD (computer-aided design) is where almost every physical product begins: an engine, a phone mid-frame, a single bolt — all of it becomes an exact parametric model in CAD before anything gets manufactured. It’s a market on the order of $12 billion, carved up for years among a handful of incumbents — Dassault (SOLIDWORKS / CATIA), Autodesk (AutoCAD / Fusion 360), Siemens (NX), PTC (Creo). Over the past decade AI has worked its way into nearly every layer of software engineering, but the hardware-design end has barely moved: engineers still model line by line, dimension by dimension, with AI relegated to simulation and layout.
