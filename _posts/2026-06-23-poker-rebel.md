...
先把丑话说前头：这是个业余 side project，不是我的正经 research。但有意思的是，“怎么老老实实做实验”这一课，它教得比我手上大多数正经实验都狠——所以值得记一笔。
...
再往前一步是 **R-NaD**（Regularized Nash Dynamics，正则化纳什动态——DeepMind 解 Stratego 用的那套）。它才是真正意义上“拿 RL 跑出 GTO 效果”的做法：照样自对弈做策略提升，但每一步都用 KL 把策略往一个**定期冻结的“磁铁”快照**上拽回去。关键就在这个正则项——有它，迭代才会收敛到 **last-iterate** 的纳什，而不是一个随时会被剥削的 best response。
...
往上一街是**转牌**。转牌后面还有河牌，叶子值不精确——这就需要一个 **value net**（价值网络）：我用河牌的精确解当监督标签，训了一个网，给定（牌面、双方 range、SPR）直接预测河牌子博弈的价值。在没见过的牌面上，它把河牌 CFV 预测到约 **3.5% 底池**的误差（解释了 87% 的信号）。转牌重解就拿它当叶子——相当于白嫖了“每个叶子都把河牌重解一遍”的价值，却不用付那份代价。再往上，同样的套路可以一路叠到翻牌（flop）。
...
把同一套 R-NaD 做成跟人数无关的（位置、在场人数都当特征喂进去），就能训 6 人、8 人桌。8 人桌的 bot 稳赢各路 baseline。但得说句实在话：**3 人以上是没有纳什保证的**（多人博弈求均衡是 PPAD-hard），所以这里只能说“经验上挺强”，没法像单挑那样拿 exploitability 说话。
